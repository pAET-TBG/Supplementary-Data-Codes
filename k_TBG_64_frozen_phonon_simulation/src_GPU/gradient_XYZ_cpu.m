function [Projs,params,errR,model,grad_x_set,grad_y_set,grad_z_set, Grad_model] = ...
    gradient_XYZ_cpu(para, xdata, ydata)
...
Grad_model = zeros(3,num_atom,'single');
...
for iter=1:iterations
    ...
    for i=1:num_pj
        ...
        % if i==2
        %     X_round(1,1,9842) = X_round(1,1,9842) - 1;
        % end
        ...
        xj_j = single(R2_Dx + R(1,3) * sum_Dz_hb );
        yj_j = single(R2_Dy + R(2,3) * sum_Dz_hb );
        zj_j = single(R2_Dz + R(3,3) * sum_Dz_hb );
        % (5,1837), (2,9842)
        % if(i==2)
        %     fprintf('%f, %f\n', X_round(1,1,9842), X_cen(1,1,9842))
        % end
        % if(i==5)
        %     % R2_Dx(:,:,9842)
        %     % xj_j(:,:,9842)
        %     % sum_Dz_hb(:,:,9842)
        %     % sum_Dz_exp(:,:,9842)      % (9,9,N)
        %     % exp_l2_z_b(:,:,9842)     % (1,9,N) agree
        %     % Dz(:,:,9842)                % (1,9,N) agree
        %     % Dz_exp_l2_z_b(:,:,9842)       %agree
        %     % sum(Dz_exp_l2_z_b(:,:,9842))    %agree
        %     % exp_l2_xy_b(:,:,9842)             % No
        %     % l2_xy(:,:,9842)                     % no
        %     % Dx(:,:,9842)            %no
        %     fprintf('%f, %f\n', X_round(1,1,1837), X_cen(1,1,1837))
        %     % [X(1,1,9842);Y(1,1,9842);Z(1,1,9842)]
        %     % [R(1,1), R(2,1), R(3,1)]
        %     % size(Dz)
        %     % size(exp_l2_z_b)
        % end
    ...
    for k=1:num_atom
        ...
        Grad_model(:,k) = [grad_x_k;grad_y_k;grad_z_k];
