...
addpath([pwd, '/src/'])
atompos_1 = importdata('atom_tracing_model_refinement_total_2D_Tracing.mat');
atompos_2 = importdata('atom_tracing_model_refinement.mat');
atompos_2 = atompos_2.atom;
atompos_2(1,:) = atompos_2(1,:)+1.0419;
atompos_2(2,:) = atompos_2(2,:)-1.4046;
atompos_2(3,:) = atompos_2(3,:)+0.0735;
...
atom_temp = [];
for i = 1:size(atompos_2,2)
    dif=(atompos_1-atompos_2(:,i));                                        % calculate all difference from the first result with i'th position in the second result (angstrom)
    dis=sqrt(sum(dif.^2,1));   
    [dis,ind]=min(dis);                                                    % obatin the minimum distance and corresponding index
    if dis <= 2
        atom_temp = [atom_temp atompos_1(:,ind)];
        atompos_1(:,ind) = [0 0 0]';
    end
end
atompos_1 = atom_temp;
...
figure(12);
% Adjust its size
set(figure(12), 'Position', [1, 1, 500*1.7, 450*1.7]);hold on;
histogram(disarr12.*100,40,'FaceColor',[0 0.4470 0.7410],'EdgeColor', 'k','LineWidth',5/1.5,'BinWidth', 1.155/2*3);hold on;
xlim([0  45]);ylim([0 1200]);                                               % Set the x-axis limits from 0 to 50
xticks(0:15:45);yticks(0:300:1200);
