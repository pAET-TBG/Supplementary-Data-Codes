...
projections = importdata('Projections_Initial_Crop.mat');
angles      = importdata('Angles_Initial.mat');
atom_info   = importdata('atom_label_Initial_Crop.mat');
model       = atom_info.atom;
model_org   = atom_info.atom;
atoms       = atom_info.label;
...
projections_total = My_paddzero(projections,size(projections) + [50 50 0],'double');
angles_total      = angles;
clear angles projections
% store the parameter for each projection
paraTotal = zeros(2,2,size(projections_total,3));

for tempIndex = 1:size(projections_total,3)
    model          = model_org;
    projections    = projections_total(:,:,tempIndex);
    angles         = angles_total(tempIndex,:);
    ...
    %% for 1226
    para0 = [1     2;  
             6.5 6.5];                                                         % the initial parameter for the fitted function
    lb = [0   0;
          6   6];                                                              % the lower bound of each paremeter
    ub = [2   4;
          7   7];                                                              % the upper bound of each parameter
    ...
    paraTotal(:,:,tempIndex) = para0;
end
para = mean(paraTotal,3);
save([pwd,'\output_data\Volume_Parameter.mat'],'para')
